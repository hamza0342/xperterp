...
class Payment {
		...
		fields = fields.concat([
			{
				...
				// default: this.frm.doc.sales_jijjk,
				onchange: () => {
					let sales_person = this.dialog.get_value('sales_person');
					me.frm.doc.sales_jijjk = sales_person
				},
				get_query: () => {
					return {
						filters: {
	...
	update_sales_person_value(fieldname, value) {
		var me = this;
		$.each(this.frm.doc.sales_team, function (i, data) {
			frappe.model.set_value('Sales Team', data.name, fieldname, value);
			frappe.model.set_value('Sales Team', data.name, 'allocated_percentage', 100);
		});
	}
