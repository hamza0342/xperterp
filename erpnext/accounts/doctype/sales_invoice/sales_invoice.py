...
class SalesInvoice(SellingController):
	...
	def validate(self):
		...
		if self.sales_jijjk and not self.sales_team:
			commission_rates_ = int(frappe.db.get_value('Sales Person', self.sales_jijjk, 'commission_rate')[:-1])
			if not commission_rates_:
				commission_rates_ = 0
			self.append('sales_team', {
				'sales_person': self.sales_jijjk,
				'allocated_percentage': 100,
				'commission_rate' : frappe.db.get_value('Sales Person', self.sales_jijjk, 'commission_rate'),
				'allocated_amount' : self.base_total,
				'incentives' : (self.base_total)/100 * commission_rates_
